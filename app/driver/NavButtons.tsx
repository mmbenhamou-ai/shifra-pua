...
export default function NavButtons({ address, label, compact = false }: NavButtonsProps) {
  ...
  const gmapsUrl = buildGoogleMapsUrl(address);
  const invalid  = !wazeUrl || !gmapsUrl;
  const tooltip  = invalid ? 'Adresse manquante' : undefined;
    ...
    return (
      <div className="flex gap-1.5">
        <a
          ...
          style={{ backgroundColor: '#E0F5FF', color: '#00A0DC', opacity: invalid ? 0.4 : 1 }}
          title={tooltip ?? 'Waze'}
        ...
        <a
          ...
          rel="noopener noreferrer"
          className="flex h-9 w-9 items-center justify-center rounded-full text-sm transition active:opacity-70"
          style={{ backgroundColor: '#EEF2FF', color: '#1A73E8', opacity: invalid ? 0.4 : 1 }}
          title={tooltip ?? 'Google Maps'}
          aria-disabled={invalid}
          onClick={(e) => {
            if (invalid) e.preventDefault();
          }}
  ...
  return (
    <div className="flex gap-2 mt-2">
      <a
        ...
        style={{ borderColor: '#B3E5F8', color: '#00A0DC', backgroundColor: '#F0FAFF', opacity: invalid ? 0.4 : 1 }}
        title={tooltip ?? 'Waze'}
        aria-disabled={invalid}
        onClick={(e) => {
          if (invalid) e.preventDefault();
      ...
      <a
        ...
        rel="noopener noreferrer"
        className="flex min-h-[42px] flex-1 items-center justify-center gap-1.5 rounded-full border text-xs font-semibold transition active:opacity-70"
        style={{ borderColor: '#C5D8FC', color: '#1A73E8', backgroundColor: '#F0F4FF', opacity: invalid ? 0.4 : 1 }}
        title={tooltip ?? (label ?? 'מפות')}
        aria-disabled={invalid}
        onClick={(e) => {
          if (invalid) e.preventDefault();
        }}
