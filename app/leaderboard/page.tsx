  ...
  return (
    <div className="min-h-screen pb-10" dir="rtl"
      ...
      <main className="mx-auto max-w-md px-4 py-6 space-y-6">
        <p className="text-center text-sm font-semibold" style={{ color: 'var(--brand)' }}>{monthName}</p>
        ...
        <section className="overflow-hidden rounded-2xl border border-[#F7D4E2] bg-white shadow-sm">
          <div className="flex items-center gap-2 border-b border-[#FBE4F0] bg-[#FBE4F0] px-5 py-3">
            ...
            <h2 className="font-bold" style={{ color: 'var(--brand)' }}>מבשלות המובילות</h2>
          </div>
          {topCooks.length === 0 ? (
              ...
              {topCooks.map((c, i) => (
                <li key={c.id} className="flex items-center gap-3 px-5 py-3.5">
                  ...
                  <div className="flex h-9 w-9 items-center justify-center rounded-full text-sm font-bold text-white flex-shrink-0"
                    style={{ background: i === 0 ? 'linear-gradient(135deg,#F59E0B,#D97706)' : 'var(--brand)' }}>
                    {c.name.charAt(0)}
                  </div>
                  <div className="flex-1 text-right">
                    <p className="text-sm font-semibold text-zinc-900">{c.name}</p>
                  </div>
                  <span className="font-bold text-sm" style={{ color: 'var(--brand)' }}>{c.count} ארוחות</span>
                </li>
              ))}
            </ol>
          )}
        ...
        <section className="overflow-hidden rounded-2xl border border-[#F7D4E2] bg-white shadow-sm">
          <div className="flex items-center gap-2 border-b border-[#FBE4F0] bg-[#FBE4F0] px-5 py-3">
            <span className="text-xl">🚗</span>
            <h2 className="font-bold" style={{ color: 'var(--brand)' }}>מחלקות המובילות</h2>
          </div>
          {topDrivers.length === 0 ? (
            <p className="px-5 py-6 text-center text-sm text-zinc-400">אין נתונים עדיין</p>
          ) : (
            <ol className="divide-y divide-[#FBE4F0]">
              {topDrivers.map((d, i) => (
                <li key={d.id} className="flex items-center gap-3 px-5 py-3.5">
                  ...
                  <span className="font-bold text-sm" style={{ color: 'var(--brand)' }}>{d.count} משלוחים</span>
                </li>
              ))}
            </ol>
          )}
